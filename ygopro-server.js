...
(function() {
  ...
  init = async function() {
    ...
    if (settings.modules.mysql.enabled) {
      global.PrimaryKeyType = settings.modules.mysql.db.type === 'sqlite' || settings.modules.mysql.db.type === 'sqljs' ? 'integer' : 'bigint';
      global.DbDateType = settings.modules.mysql.db.type === 'postgres' ? 'timestamp' : 'datetime';
    ...
    if (settings.modules.random_duel.enabled) {
      setInterval(async function() {
        ...
        for (l = 0, len1 = ROOM_all.length; l < len1; l++) {
          room = ROOM_all[l];
          if (!(room && room.duel_stage !== ygopro.constants.DUEL_STAGE.BEGIN && room.random_type && room.last_active_time && room.waiting_for_player && room.get_disconnected_count() === 0 && room.duel_stage !== ygopro.constants.DUEL_STAGE.SIDING && !room.recovered)) {
            continue;
          }
    ...
    if (settings.modules.mycard.enabled) {
      setInterval(function() {
        ...
        for (l = 0, len1 = ROOM_all.length; l < len1; l++) {
          room = ROOM_all[l];
          if (!(room && room.duel_stage !== ygopro.constants.DUEL_STAGE.BEGIN && room.arena && room.last_active_time && room.waiting_for_player && room.get_disconnected_count() === 0 && room.duel_stage !== ygopro.constants.DUEL_STAGE.SIDING && !room.recovered)) {
            continue;
          }
          time_passed = Math.floor(moment_now.diff(room.last_active_time) / 1000);
  ...
  ROOM_find_or_create_by_name = global.ROOM_find_or_create_by_name = async function(name, player_ip) {
    ...
    if (settings.modules.random_duel.enabled && (uname === '' || uname === 'S' || uname === 'M' || uname === 'T' || uname === 'TT' || settings.modules.random_duel.extra_modes[uname] !== void 0)) {
  ...
  ROOM_find_or_create_random = global.ROOM_find_or_create_random = async function(type, player_ip) {
    ...
    result = _.find(ROOM_all, function(room) {
      var ref;
      return room && room.random_type !== '' && (!room.disconnector || type === 'TT') && room.duel_stage === ygopro.constants.DUEL_STAGE.BEGIN && !room.windbot && ((type === '' && (room.random_type === settings.modules.random_duel.default_type || settings.modules.random_duel.blank_pass_modes[room.random_type])) || room.random_type === type) && (0 < (ref = room.get_playing_player().length) && ref < max_player) && (type === 'TT' || settings.modules.random_duel.no_rematch_check || room.get_host() === null || room.get_host().ip !== ROOM_players_oppentlist[player_ip]) && (type === 'TT' || playerbanned === room.deprecated || type === 'T');
    });
    if (result) {
    ...
    //log.info 'found room', player_name
      ...
      // 天梯(TT):房名带 M# 前缀,让 Room 构造函数在 spawn 子进程前解析为 MATCH 比赛模式(mode=1)
      name = (type === 'TT' ? 'M#' : '') + type + ',RANDOM#' + Math.floor(Math.random() * 100000);
    ...
    } else if (result.random_type === 'TT') {
      result.welcome2 = '天梯模式:比赛决斗,不计入约战,计入天梯战绩';
    } else {
  ...
  Room = class Room {
    ...
    delete() {
      ...
      // 天梯结算:仅 TT(天梯)随机模式,计入注册与胜/负场
      if (settings.modules.mysql.enabled && this.random_type === 'TT' && score_array.length === 2) {
        if (score_array[0].score !== score_array[1].score) {
          var lw_form, ll_form, lmd_key;
          if (score_array[0].score > score_array[1].score) {
            lw_form = score_array[0];
            ll_form = score_array[1];
          } else {
            lw_form = score_array[1];
            ll_form = score_array[0];
          }
  ...
  ygopro.ctos_follow('JOIN_GAME', true, async function(buffer, info, client, server, datas) {
    ...
    if (CLIENT_is_able_to_reconnect(client) || CLIENT_is_able_to_kick_reconnect(client)) {
      ...
      await polyfill_version();
      // 天梯(TT)入口校验:只允许"已注册且密码正确"或"首次创建(未注册)"的玩家进入;无密码/密码错误直接拒绝
      if (settings.modules.random_duel.enabled && info.pass.toUpperCase() === 'TT' && settings.modules.mysql.enabled) {
        if (!client.vpass) {
          ygopro.stoc_die(client, '天梯模式必须使用 昵称$密码 格式');
          return;
        }
        tt_check_user = (await dataManager.getLadderUser(client.name));
        if (tt_check_user && tt_check_user.pass !== client.vpass) {
          ygopro.stoc_die(client, '用户名或密码错误');
          return;
        }
      }
      //log.info 'join_game',info.pass, client.name
      room = (await ROOM_find_or_create_by_name(info.pass, client.ip));
      // 天梯房直连(输入具体房名,而非 TT 代码):只能观战,永远不能进对战席
      if (room && room.random_type === 'TT' && info.pass.toUpperCase() !== 'TT') {
        if (room.duel_stage === ygopro.constants.DUEL_STAGE.BEGIN) {
          ygopro.stoc_die(client, '该房间是天梯对局,无法直接加入;对局开始后可输入房名观战');
        } else {
  ...
  ygopro.stoc_follow('HS_PLAYER_CHANGE', true, async function(buffer, info, client, server, datas) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (player.pos === pos) {
        ...
        if (is_ready && settings.modules.mysql.enabled && room.random_type === 'TT') {
          // 天梯打钩:首次创建(未注册)的玩家提示注册时机
          (async () => {
            try {
              const lu = await dataManager.getLadderUser(player.name);
              if (!lu) {
                ygopro.stoc_send_chat(player, '首次参与天梯:本场对决结束将注册您的账号,此后使用该密码参与并计入战绩', ygopro.constants.COLORS.BABYBLUE);
              }
            } catch (err) {
              log.warn('LADDER CHECK FAIL', err.toString());
            }
          })();
        }
      }
  ...
  ygopro.ctos_follow('SURRENDER', true, async function(buffer, info, client, server, datas) {
    ...
    if (room.random_type && room.turn < 3 && !client.flee_free && !settings.modules.test_mode.surrender_anytime && room.random_type !== 'TT' && !(room.random_type === 'M' && settings.modules.random_duel.record_match_scores)) {
  ...
  ygopro.ctos_follow('CHAT', true, async function(buffer, info, client, server, datas) {
    ...
    switch (cmd[0]) {
      ...
      case '/surrender':
        ...
        if (room.random_type && room.turn < 3 && !client.flee_free && room.random_type !== 'TT') {
          ygopro.stoc_send_chat(client, "${surrender_denied}", ygopro.constants.COLORS.BABYBLUE);
          return cancel;
        }
        if (client.surrend_confirm) {
  ...
  ygopro.stoc_follow('REPLAY', true, async function(buffer, info, client, server, datas) {
    ...
    if (room && client.pos === 0 && !room.replays[room.duel_count - 1]) {
      ...
      if (room.random_type === 'TT' || room.has_ygopro_error) {
        //console.log('save replay')
        replay_filename = moment_now.format("YYYY-MM-DD HH-mm-ss.SSS");
        if (room.hostinfo.mode !== 2) {
        ...
        if (settings.modules.mysql.enabled && room.random_type === 'TT') {
  ...
  if (true) {
    ...
    httpRequestListener = async function(request, response) {
      ...
      if (u.pathname === '/' || u.pathname === '/intro.html' || u.pathname === '/rooms.html' || u.pathname === '/replays.html' || u.pathname === '/ladder.html' || u.pathname === '/deck-stats.html' || u.pathname === '/dashboard.html') {
        ...
        try {
          var webPageData = await fs.promises.readFile(path.join(__dirname, 'web', webPageFile));
          response.writeHead(200, {
            "Content-Type": "text/html; charset=utf-8"
          });
          response.end(webPageData);
        } catch (err2) {
      ...
      // ===== 定制:录像文件列表(免登录,分页,关键词搜索) =====
      if (u.pathname === '/api/replays') {
        try {
          var replayFiles = await fs.promises.readdir(settings.modules.tournament_mode.replay_path);
          var replaysSearch = (u.query.search || '').trim().toLowerCase();
          var allReplayNames = [];
          for (var rfi = 0, rfl = replayFiles.length; rfi < rfl; rfi++) {
            var replayFile = replayFiles[rfi];
            if (replayFile.toLowerCase().endsWith('.yrp') && (!replaysSearch || replayFile.toLowerCase().indexOf(replaysSearch) !== -1)) {
              allReplayNames.push(replayFile);
            }
          }
          // 文件名以 "YYYY-MM-DD HH-mm-ss.SSS " 开头 → 字典序即时间序(降序 = 最新在前),无需逐个 stat
          allReplayNames.sort().reverse();
          var replaysTotal = allReplayNames.length;
          var replaysPageSize = parseInt(u.query.pageSize) || 20;
          if (replaysPageSize < 1) {
            replaysPageSize = 20;
          }
          if (replaysPageSize > 100) {
            replaysPageSize = 100;
          }
          var replaysPage = parseInt(u.query.page) || 1;
          if (replaysPage < 1) {
            replaysPage = 1;
          }
          ...
          for (var rfi2 = 0, rfl2 = replaysPageNames.length; rfi2 < rfl2; rfi2++) {
            ...
            var replayStat = await fs.promises.stat(path.join(settings.modules.tournament_mode.replay_path, replayFile2));
            replayList.push({
              name: replayFile2,
      ...
      if (u.pathname === '/api/getrooms') {
        // 仅当请求显式携带账密时才做管理员鉴权(避免免登录访问刷"Unknown user login"日志)
        if (u.query.username) {
      ...
      } else if (_.startsWith(u.pathname, '/api/replay')) {
        // 定制:录像下载公开(免登录),文件名经 basename 防路径穿越
        {
          getpath = null;
          filename = null;
          try {
